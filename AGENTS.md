...
3.  数据库 migration 必须通过 `drizzle-kit generate` 或仓库封装命令生成，不要手写 SQL migration，也不要手动改动 `drizzle/meta` 下的数据，它们都应该是命令生成的。
...
6. UI 组件默认不要展示面向用户的实现性说明文案，例如“支持按账户、交易所、状态、币种和时间范围筛选，并可下钻到对应账户详情”或 “Chronological feed of all synced articles” 这类功能解释；界面应尽量简洁，只保留必要的信息层级与操作提示。除非用户明确要求，否则不要额外添加副标题式说明、功能概述或实现解释。
7. 数据库中默认是UTC时间（或unix时间戳）, 前端显示和业务（比如按天统计）都用配置的时区
