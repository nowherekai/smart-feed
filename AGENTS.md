...
2. 后端需要添加足够的log
3. TypeScript 中尽量不要使用 `any`；优先使用具体类型、泛型、联合类型或 `unknown`。
4.  数据库 migration 必须通过 `drizzle-kit generate` 或仓库封装命令生成，不要手写 SQL migration，也不要手动改动 `drizzle/meta` 下的数据，它们都应该是命令生成的。
5. 一旦某个 migration 文件已经提交，或有可能已被任何环境执行过，就禁止再修改该 migration 的 SQL、文件名、序号、snapshot 与 `_journal.json` 对应记录；后续 schema 变更只能追加新的 migration。
