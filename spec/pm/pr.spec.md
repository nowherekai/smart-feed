...
* **核心环境定义**: 为知识工作者提供一个可控、可追溯、可调优的信息处理空间。系统需完成信息获取、清洗标准化、AI 分析与 digest 编排，并将结果以日报形式投递。

### 1.2 主要行动者 (Primary Agents)

| Agent | 类型 | 主要目标 | 感知方式 (Signifiers) |
| :--- | :--- | :--- | :--- |
| **人类用户** | Human | 配置来源、消费情报、建立信任、持续调优输出 | 来源配置入口、邮件/报告、原文回链、证据片段、反馈入口 |
| **AI 分析代理** | AI | 分类、抽取、摘要、价值判断，满足可追溯与成本约束 | 标准化内容模型、来源元数据、分析缓存、反馈信号、约束规则 |
...
* **P5 可追溯阅读**: 允许用户从结论回到来源名称、原文链接与证据片段。
...
### MAS-2: 智能分析与可追溯摘要
* **主题**: AI 自动分析内容价值，生成带有证据链的摘要。
* **核心序列**:
    1. 系统执行轻量分析（分类、关键词、价值评分）。
    2. 对高价值内容触发摘要生成（一句话、三点要点、关注理由）。
    3. **追溯性注入**: 每条摘要必须关联原始链接与原文证据片段。
...
| **数据分离** | 原始数据（Raw HTML）与加工数据（Markdown/Analysis）必须分开管理。 |
...
命名约定：`id` / `source_id` / `content_id` 表示内部 UUID 或外键；`identifier` 表示规范化 URL；面向用户展示与 traceability 使用 `source_trace_id` / `content_trace_id`（可由业务字段派生，MVP 不要求单独持久化）。
...
* **AC-3**: 每一条生成的摘要下方必须显式包含 `original_url` 和至少一段 `evidence_snippet`。
