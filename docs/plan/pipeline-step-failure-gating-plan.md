...
- 基础分析成功：
  - `outcome = completed`
  - 后续是否 enqueue heavy 取决于 `value_score > threshold`
...
## Action items
[ ] 新增 `src/pipeline/types.ts` 或等价模块，定义统一步骤结果协议与类型约束。
[ ] 新增 `src/services/pipeline-runtime.ts`，封装 `pipeline_run` / `step_run` 生命周期、错误归一化和下一步入队控制。
[ ] 重构 `src/services/content.ts`，移除内部 enqueue，把 `content.fetch-html` / `content.normalize` 改成返回统一步骤结果。
[ ] 重构 `src/pipeline/handlers/content-fetch-html.ts` 与 `src/pipeline/handlers/content-normalize.ts`，通过 runtime 执行并在成功时推进下一步。
[ ] 新增 `src/pipeline/handlers/content-analyze-basic.ts`，最小接通 AI 适配层和统一失败阻断逻辑。
[ ] 修改 `src/pipeline/handlers/index.ts`，用真实 `content.analyze.basic` handler 替换 placeholder。
[ ] 为 `pipeline runtime`、`content-fetch-html`、`content-normalize`、`content-analyze-basic` 增加单测，覆盖 success / fallback / failed 三类结果。
[ ] 验证“当前 content 失败不推进下一步，但其它 content/source job 不受影响”的行为。
[ ] 运行 `bun run check`、`bun test`、`bun run build`，确保现有 Task 1-4 不回归。
[ ] 更新 `docs/plan/backend-implementation-plan.md` 与新的 change doc，明确失败阻断模型已成为通用规则而非 AI 特例。
...
- `pipeline_run` 在 content 链路里是“每个 job 一个 run”还是“整个 content 生命周期一个 run”？本计划建议先做“整个 content 生命周期一个 run”，实现时若成本过高，可先退回“每个 handler 一个 run”，但要在文档里明确。
- `content.analyze.basic` 在本轮是否只接通最小 AI 可用性检查，还是顺手实现 analysis_record 落库？本计划建议先以统一失败阻断为主，不强制同时完成完整落库。
- `source.fetch` 是否本轮一起接入统一 runtime？本计划建议先优先收敛 content 链路，再视时间把 source 链路纳入同一模型。
